...
export default function App() {
  ...
  // Agregamos la comprobación para evitar errores al usar split
  ...
  const filteredStays = stays.filter(st => {
    const locationIsProvided = city[0].trim() !== ""; // Verifica si se proporcionó una ciudad
    const totalGuests = invitados; // Total de invitados
  
    const matchesCity = locationIsProvided 
      ? (() => {
          return (
            (st.city === city[0] || city[0] === "Whole") // Compara la ciudad y verifica si es "Whole"
          );
        })()
      : true;
                                   
    const matchesGuests = (totalGuests === 0 || (st.maxGuests || 0) >= totalGuests); // Verifica la cantidad de invitados
  
    return matchesCity && matchesGuests; // Retorna el resultado final
  });        
